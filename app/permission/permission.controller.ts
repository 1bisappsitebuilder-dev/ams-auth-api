...
export const controller = (prisma: PrismaClient) => {
	const getById = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			if (!idValidation.success) {
				permissionLogger.error(config.ERROR.QUERY_PARAMS.INVALID_ID);
			...
			if (error.name === "PrismaClientValidationError") {
				...
				permissionLogger.error(`${config.ERROR.USER.ERROR_GETTING_USER}: ${errorMsg}`);
				res.status(400).json(buildErrorResponse(errorMsg, 400));
			} else {
				permissionLogger.error(`${config.ERROR.USER.ERROR_GETTING_USER}: ${error}`);
				res.status(500).json(
	...
	const getAll = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			const filterConditions = buildFilterConditions("Permission", filter);
